...
import scalacache.MonadError

class Macros(val c: blackbox.Context) {
  import c.universe._

  def memoize[F[_], M[G[_]] <: MonadError[G], V](cache: c.Tree, ttl: c.Tree)(f: c.Tree)(mode: c.Tree): Tree = {
    commonMacroImpl(cache, { keyName =>
      q"""$cache.caching($keyName)($ttl)($f)($mode)"""
    })
  }

  def memoizeF[F[_], M[G[_]] <: MonadError[G], V](cache: c.Tree, ttl: c.Tree)(f: c.Tree)(mode: c.Tree): Tree = {
    commonMacroImpl(cache, { keyName =>
  ...
  private def commonMacroImpl[F[_], M[G[_]] <: MonadError[G], V: c.WeakTypeTag](cache: c.Tree, keyNameToCachingCall: (c.TermName) => c.Tree): Tree = {
