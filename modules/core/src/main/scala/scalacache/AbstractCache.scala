...
trait AbstractCache[V, M[F[_]] <: MonadError[F]] extends CacheAlg[V, M]{

  def config: CacheConfig

  def getWithKey[F[_]](key: String)(implicit mode: Mode[F, M]): F[Option[V]]

  def putWithKey[F[_]](key: String, value: V, ttl: Option[Duration])(implicit mode: Mode[F, M]): F[Unit]
